    ...
    for max_iter in const.MAX_ITERS:
        for wait_iter in const.WAIT_ITERS:
            for dependency_mode in const.DEPENDENCY_MODES:
                generator = Generator(max_iter=max_iter, wait_iter=wait_iter, dependency_mode=dependency_mode, test_type=test_type)
                (ct, generations_cnt) = generator.generate_tests(code)
                print(str(generator), "->", ct.calculate_coverage(), generations_cnt, sep=" ")
    ...
    for pop_size in const.POP_SIZES:
        for rate_crossover in const.RATES_CROSSOVER:
            for rate_mutation in const.RATES_MUTATION:
                generator = Generator(population_size=pop_size, rate_crossover=rate_crossover, rate_mutation=rate_mutation, test_type=test_type)
                (ct, generations_cnt) = generator.generate_tests(code)
                print(str(generator), "->", ct.calculate_coverage(), generations_cnt, sep=" ")
            ...
            for rate_crossover in const.RATES_CROSSOVER:
                for rate_mutation in const.RATES_MUTATION:
                    for max_iter in const.MAX_ITERS:
                        for wait_iter in const.WAIT_ITERS:
                            for dependency_mode in const.DEPENDENCY_MODES:
                                generator = Generator(seed_size, pop_size, rate_crossover, rate_mutation, max_iter, wait_iter, dependency_mode, test_type)
                                (ct, generations_cnt) = generator.generate_tests(code)
                                print(str(generator), "->", ct.calculate_coverage(), generations_cnt, sep=" ")
